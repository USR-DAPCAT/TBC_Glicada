...
# HBA1C entre c(3-20)
# recode_to_missings()
dades_long<-dades_long %>% mutate(val=ifelse(val<3 | val>20,NA,val))
...
dades_long<-dades_long %>% left_join(select(dades,-situacio),by=c("idp"="CIP"))
...
```{r, include=TRUE}
vars_ajust<-
  extreure.variables("ajuste1",conductor_variables) 
vars_ajust<-vars_ajust[!vars_ajust%in%("HbA1c_7.5")] %>% c("DG.TBC_cat")


fit<-coxph(Surv(tstart, tstop, situacio=="TBC") ~ val_7_5 + 
             sexe + age + DM_anys + MACRO + MICRO + DG.TAB_cat + DG.ALCOHOL_cat + IMC.valor + DG.TBC_cat +
