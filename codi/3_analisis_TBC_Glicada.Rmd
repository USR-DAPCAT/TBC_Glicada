...
#prova<-dades_long%>%filter((CIP=="GUJA033071500" | 
#                            CIP=="AAAO141052000" | 
#                            CIP=="PERU061052700" | 
#                            CIP=="GAFE034052000") & cod=="HBA1c") %>%arrange(CIP,cod,dat)#
#
#
#
#   dt=prova  
...
# dades_long$sexe
# dades_long$age
fit1 <- coxph(Surv(tstart, tstop, situacio=="TBC") ~ val_7_5 + cluster(idp),data=dades_long)
sjPlot::tab_model(fit1)
cox.zph(fit1)

fit2 <- coxph(Surv(tstart, tstop, situacio=="TBC") ~ val_8 + cluster(idp),data=dades_long)
sjPlot::tab_model(fit2)
cox.zph(fit2)

fit3 <- coxph(Surv(tstart, tstop, situacio=="TBC") ~ val_9 + cluster(idp),data=dades_long)
sjPlot::tab_model(fit3)
cox.zph(fit3)

fit4 <- coxph(Surv(tstart, tstop, situacio=="TBC") ~  val_cat + cluster(idp),data=dades_long)
sjPlot::tab_model(fit4)
cox.zph(fit4)

fit5 <- coxph(Surv(tstart, tstop, situacio=="TBC") ~  val + cluster(idp),data=dades_long)
...
```{r, comprovacio, eval=FALSE}



fit <- coxph(Surv(tstart, tstop, situacio=="TBC") ~ val_7_5 + cluster(idp),data=dades_long)
#sjPlot::tab_model(fit)
fitB <- coxph(Surv(tstart, tstop, situacio=="TBC") ~ val_7_5 ,data=dades_long)
...
# Hi hagut un error de filtres, no podia ser que tinguèssim tstart>=tfinal, no haviem acotat la matriu , per la
#la DataFinal, la data final Estudi! ( canvi, mort o Tbc!!).

# sempre hem d'aplicar coxph(Surv(tstart,tstop,situacio=="TBC", no cal posar Cluster!!!))
