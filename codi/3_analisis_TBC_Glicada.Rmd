...
dades_long2<-dades_long2 %>% mutate(val_7_5 = ifelse(val<7.5    , 0, 1))
dades_long2<-dades_long2 %>% mutate(val_8 =   ifelse(val<8      , 0, 1))
dades_long2<-dades_long2 %>% mutate(val_9 =   ifelse(val<9      , 0, 1))
...
fit1 <- coxph(Surv(tstart, tstop, situacio=="TBC") ~ val+cluster(idp),data=dades_long2)
summary(fit1)
#sjPlot::tab_model(fit1)
cox.zph(fit1)


fit2 <- coxph(Surv(tstart, tstop, situacio=="TBC") ~ val_7_5+cluster(idp),data=dades_long2)
summary(fit2)
#sjPlot::tab_model(fit2)
cox.zph(fit2)

fit3 <- coxph(Surv(tstart, tstop, situacio=="TBC") ~ val_8+cluster(idp),data=dades_long2)
summary(fit3)
#sjPlot::tab_model(fit3)
cox.zph(fit3)

fit4 <- coxph(Surv(tstart, tstop, situacio=="TBC") ~ val_9+cluster(idp),data=dades_long2)
summary(fit4)
#sjPlot::tab_model(fit4)
cox.zph(fit4)

fit5 <- coxph(Surv(tstart, tstop, situacio=="TBC") ~  pspline(val,2) + +cluster(idp),data=dades_long2)
summary(fit5)
...
Desde un punt de vista clínic m'agradaria fer 2 models multivariants

a) Ajustant només per edat i sexe
...
```{r resultats5, message=FALSE, warning=FALSE, include=T, echo=FALSE,size="huge",eval = F}
# T BASAL -------------
...
res1b<-dades %>% Resum_taxa_incidencia(evento="event_tbc",temps="anys_lliure_tbc",valorevent="1") 
res1b<-tibble::tibble(Group="Overall") %>% bind_cols(res1b)

res5<-dades %>% split(.$HbA1c_7.5) %>% map_dfr(~Resum_taxa_incidencia(.x,evento="event_tbc",temps="anys_lliure_tbc",valorevent="1"),.id = "Group") 
res6<-dades %>% split(.$HbA1c_9) %>% map_dfr(~Resum_taxa_incidencia(.x,evento="event_tbc",temps="anys_lliure_tbc",valorevent="1"),.id = "Group") 
...
# Models de COX 
taula_models_COX<-vector_ajustos %>% purrr::map_df(~extreure_HR(a="HbA1c_7.5",x=.x,event = "event_tbc",t="temps_tbc",d=dades,taulavariables = conductor_variables,c=""),.id="Model")
taula_models_COX%>% 
...
# Models de COX 
taula_models_COX<-vector_ajustos %>% purrr::map_df(~extreure_HR(a="HbA1c_9",x=.x,event = "event_tbc",t="temps_tbc",d=dades,taulavariables = conductor_variables,c=""),.id="Model")
taula_models_COX%>% 
  kable(digits = 3,caption = "Analisis Multivariante de los factors con pacientes DM  que pueden  influir a la TBC") %>% 
  kableExtra::kable_styling()
...
res.cox3<-coxph(surv_tbc ~ sexe+age+DM_anys+MACRO+DG.TAB_cat+DG.ALCOHOL_cat+IMC.valor+HbA1c_7.5, data=dades) 
...
res.cox4<-coxph(surv_tbc ~ sexe+age+DM_anys+MACRO+DG.TAB_cat+DG.ALCOHOL_cat+HbA1c_7.5, data=dades) 
...
res.cox5<-coxph(surv_tbc ~ sexe+age+DM_anys+MACRO+DG.ALCOHOL_cat+IMC.valor+HbA1c_7.5, data=dades) 
...
res.cox6<-res.cox<-coxph(surv_tbc ~ sexe+age+DM_anys+MACRO+DG.TAB_cat+IMC.valor+HbA1c_7.5, data=dades) 
...
res.cox7<-res.cox<-coxph(surv_tbc ~ sexe+age+DM_anys+MACRO+HbA1c_7.5, data=dades) 
