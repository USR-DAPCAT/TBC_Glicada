...
#     Problemes de salut ----------
dt_psalut3<-
  read.csv2(here::here("dades","PSalut3.csv"), header=T, sep=";",fileEncoding="utf-16") %>% as_tibble()

dt_psalut<-
  read.csv(here::here("dades","extracció tbc dm metode antic-1.csv"), header=T,
           sep=";",fileEncoding="utf-16") %>% as_tibble()


# Fusió problemes
dt_psalut<-dt_psalut %>% 
  bind_rows(dt_psalut3) %>% select(cipACT,codiPSalut,ddeteccio)
rm(dt_psalut3)
...
```{r lectura2, warning=F, cache=TRUE}
...
#TB _PULMONAR"    "TB_PLEURAL"      "TB_PLEUROPULM"   "TB_LIMFATICA"    "TB_OSTEOART"     "TB_MILIAR"     "TB_GENITOURIN" 
# "TB_MENINGEA"     "TB_LARINGEA"     "TB_ALTRESLOC" 
