...
```{r mostreigtest}
if (params$test==T) {
  dt_temp<-dades %>% select(case.id,CIP) %>% sample_n(10)
  dt_temp2<-dt_temp%>% select(case.id)
  dt_temp3<-dt_temp%>% select(CIP)
  dades<-dades %>% semi_join(dt_temp2)
  dades_long<-dades_long%>% semi_join(dt_temp3)
    }
```


```{r funcions propies, eval=TRUE}
...
) 
{
  ...
  dt<-dt%>%select(idp,cod,val,sym(!!dataini),sym(!!datasort),sym(!!endpt),sym(!!bd.dindex),HBA1c.valor)
  dt<-dt%>%arrange(idp,!!dataini)
  
  
  
  #bd.dindex=dt$dtindex
...
#dt_variables2<-dt_variables %>% mutate(idp=CIP) %>% select(-CIP) %>% 
#  left_join(dades,by="idp")%>%

  
 
#dt_variables2<-dt_variables%>%mutate(idp=CIP)%>%select(-CIP)%>%left_join(dades,by="idp")%>%
#  select(idp,sexe,dtindex,cod,dat,val,situacio,datafi,EV.TBC,HBA1c.valor)


dt_variables2<-dt_variables%>%mutate(idp=CIP)%>%left_join(dades,by="CIP")%>%
  select(CIP,sexe,dtindex,cod,dat,val,situacio,datafi,EV.TBC,HBA1c.valor)
...
dt_variables2<-dt_variables2%>%select(idp=CIP,CIP,cod,dat,val,situacio,datafi,dtindex,HBA1c.valor)
#dt_variables2<-dt_variables2%>%arrange(cod ,dat)
dt_variables2<- dt_variables2%>%filter(!is.na(val))


dt_variables2<-dt_variables2%>%arrange(idp,cod,dat)


dades_long<-dades_long%>%arrange(idp,cod,dat)
...
```{r MODEL1, include=TRUE,eval = TRUE}
...
analitiques_temps1<-Analitica_Temps(
  dt=dades_long ,  
  grup="HBA1c",
  dataini="dat",
  datasort="datafi",
...
# aliquem la funcio , dades Rai!

analitiques_temps2<-Analitica_Temps2(
  dt=dt_variables2 ,  
  grup="HBA1c",
  dataini="dat",
  datasort="datafi",
...
analitiques_temps1<-analitiques_temps1%>%left_join(dades2,by="idp")
analitiques_temps1<-analitiques_temps1%>%mutate(val_7_5 = ifelse(val<7.5    , 0, 1))
analitiques_temps1<-analitiques_temps1%>%mutate(val_8 =   ifelse(val<8      , 0, 1))
analitiques_temps1<-analitiques_temps1%>%mutate(val_9 =   ifelse(val<9      , 0, 1))
analitiques_temps1<-analitiques_temps1%>%mutate(val2 = ifelse(val<8  , 0, 1))
analitiques_temps1<-analitiques_temps1%>%mutate(val3 = ifelse(val<9  , 0, 1))
analitiques_temps1<-analitiques_temps1 %>% mutate(edat_CAT=case_when(edat<75 ~ "1.<75", edat >=75~ "2.>=75" ))
...
analitiques_temps2<-analitiques_temps2%>%left_join(dades2,by="idp")
analitiques_temps2<-analitiques_temps2%>%mutate(val_7_5 = ifelse(val<7.5    , 0, 1))
analitiques_temps2<-analitiques_temps2%>%mutate(val_8 =   ifelse(val<8      , 0, 1))
analitiques_temps2<-analitiques_temps2%>%mutate(val_9 =   ifelse(val<9      , 0, 1))
analitiques_temps2<-analitiques_temps2%>%mutate(val2 = ifelse(val<8  , 0, 1))
analitiques_temps2<-analitiques_temps2%>%mutate(val3 = ifelse(val<9  , 0, 1))
analitiques_temps2<-analitiques_temps2 %>% mutate(edat_CAT=case_when(edat<75 ~ "1.<75", edat >=75~ "2.>=75" ))
...
#i)
fit1a <- coxph(Surv(tstart, tstop, situacio=="TBC") ~ val_7_5+sexe+edat+cluster(idp),data=analitiques_temps1)
summary(fit1a)
cox.zph(fit1a)
#plot(cox.zph(fit1))

#i)
fit1b <- coxph(Surv(tstart, tstop, situacio=="TBC") ~ val_7_5+sexe+edat+cluster(idp),data=analitiques_temps2)
summary(fit1b)
cox.zph(fit1b)
#plot(cox.zph(fit1))
...
fit2a <- coxph(Surv(tstart, tstop, situacio=="TBC") ~ val_7_5+edat+strata(sexe)+cluster(idp),data=analitiques_temps1)
summary(fit2a)
cox.zph(fit2a)

fit2b <- coxph(Surv(tstart, tstop, situacio=="TBC") ~ val_7_5+edat+strata(sexe)+cluster(idp),data=analitiques_temps2)
summary(fit2b)
cox.zph(fit2b)
...
# 
fit3a <- coxph(Surv(tstart, tstop, situacio=="TBC") ~ val+edat+sexe+cluster(idp),data=analitiques_temps1)
summary(fit3a)
cox.zph(fit3a)
sjPlot::tab_model(fit3a)

# 
fit3b <- coxph(Surv(tstart, tstop, situacio=="TBC") ~ val+edat+sexe+cluster(idp),data=analitiques_temps2)
summary(fit3b)
cox.zph(fit3b)
sjPlot::tab_model(fit3b)
...
#ii)
fit4a <- coxph(Surv(tstart, tstop, situacio=="TBC") ~ val_8+sexe+edat+cluster(idp),data=analitiques_temps1)
summary(fit4a)
cox.zph(fit4a)

#ii)
fit4b <- coxph(Surv(tstart, tstop, situacio=="TBC") ~ val_8+sexe+edat+cluster(idp),data=analitiques_temps2)
summary(fit4b)
cox.zph(fit4b)


fit5a <- coxph(Surv(tstart, tstop, situacio=="TBC") ~ val_8+edat+strata(sexe)+cluster(idp),data=analitiques_temps1)
summary(fit5a)
cox.zph(fit5a)


fit5b <- coxph(Surv(tstart, tstop, situacio=="TBC") ~ val_8+edat+strata(sexe)+cluster(idp),data=analitiques_temps2)
summary(fit5b)
cox.zph(fit5b)
...
#iii) 
fit6a <- coxph(Surv(tstart, tstop, situacio=="TBC") ~ val_9+sexe+edat+cluster(idp),data=analitiques_temps1)
summary(fit6a)
cox.zph(fit6a)


#iii)
fit6b <- coxph(Surv(tstart, tstop, situacio=="TBC") ~ val_9+sexe+edat+cluster(idp),data=analitiques_temps2)
summary(fit6b)
cox.zph(fit6b)


#iv)
fit7a <- coxph(Surv(tstart, tstop, situacio=="TBC") ~ val_9+edat+strata(sexe)+cluster(idp),data=analitiques_temps1)
summary(fit7a)
cox.zph(fit7a)
#plot(cox.zph(fit4))
...
#iv)
fit7b <- coxph(Surv(tstart, tstop, situacio=="TBC") ~ val_9+edat+strata(sexe)+cluster(idp),data=analitiques_temps2)
summary(fit7b)
cox.zph(fit7b)
...
```
## Tabla.6d Modelos de Cox dependientes del Tiempo (HBA1c) para la TBC,estratificado por sexo.[Hba1c>9+strata(edat)+strata(sexe)+cluster(idp)]

```{r MODEL_COX4, include=T,eval = TRUE}




#v)
fit8a <- coxph(Surv(tstart, tstop, situacio=="TBC") ~ val_9+edat_CAT+strata(sexe)+cluster(idp),data=analitiques_temps1)
summary(fit8a)
cox.zph(fit8a)
#plot(cox.zph(fit5))
...
#v)
fit8b <- coxph(Surv(tstart, tstop, situacio=="TBC") ~ val_9+edat_CAT+strata(sexe)+cluster(idp),data=analitiques_temps2)
summary(fit8b)
cox.zph(fit8b)
